...
import com.simplemobiletools.filemanager.dialogs.ConvertFileDialog
...
class ItemsAdapter(val activity: SimpleActivity, var mItems: MutableList<FileDirItem>, val listener: ItemOperationsListener?, val isPickMultipleIntent: Boolean,
    ...
    private val multiSelector = SingleSelector()
    ...
    fun toggleItemSelection(select: Boolean, pos: Int) {
        ...
        for (index in selectedPositions){
            itemViews[index]?.item_frame?.isSelected=false;
        }
    ...
    private val multiSelectorMode = object : ModalMultiSelectorCallback(multiSelector) {
        override fun onActionItemClicked(mode: ActionMode, item: MenuItem): Boolean {
            when (item.itemId) {
                ...
                R.id.cab_convert -> convertDialog()
    ...
    private fun convertDialog() {
        ConvertFileDialog(activity, getSelectedMedia()[0].path) {
    ...
    class ViewHolder(val view: View, val adapterListener: MyAdapterListener, val activity: SimpleActivity, val multiSelectorCallback: ModalMultiSelectorCallback,
                     val multiSelector: SingleSelector, val listener: ItemOperationsListener?, val itemClick: (FileDirItem) -> (Unit)) : SwappingHolder(view, multiSelector) {
        fun bindView(fileDirItem: FileDirItem, fileDrawable: Drawable, folderDrawable: Drawable, textColor: Int): View {
            itemView.apply {
                ...
                setOnLongClickListener { viewLongClicked(fileDirItem); true }
            }
        ...
        private fun viewLongClicked(fileDirItem: FileDirItem) {
            if (listener != null) {
                if (!multiSelector.isSelectable&&!fileDirItem.isDirectory) {
                    activity.startSupportActionMode(multiSelectorCallback)
                    adapterListener.toggleItemSelectionAdapter(true, adapterPosition)
                }

                listener.itemLongClicked(fileDirItem)
            }
        }
    ...
    interface ItemOperationsListener {
        ...
        fun itemLongClicked(fileDirItem: FileDirItem)
